...
export async function GET(request: Request) {
  ...
  // Extract vps_id from the url query parameters
  const vps_id = request.url.split("?vps_id=")[1];

  if (!vps_id) {
    return NextResponse.json(
      { error: "Missing vps_id in request body" },
      { status: 400 },
    );
  }

  try {
    const response = await fetch(`${URL_CUBE_PATH}/vps/${vps_id}/metrics`, {
      method: "GET",
      headers: {
